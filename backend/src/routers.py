...
# ✅ НОВЫЙ РОУТ: Получение текущего пользователя
@routerAuth.get("/me", response_model=User)
...
async def register(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    new_user = UserModel(username=user.username, hashed_password=hashed_pwd, is_admin=False)
    db.add(new_user)
    await db.commit()
...
async def get_items(
    ...
    if status: query = query.where(ItemModel.status == status.value)
    if priority: query = query.where(ItemModel.priority == priority.value)
    if search:
        sp = f"%{search}%"
...
async def update_item(item_id: int, item_update: ItemCreate, db: AsyncSession = Depends(get_db)):
    ...
    if not item: raise HTTPException(status_code=404, detail="Заявка не найдена")
    if item.status == "done": raise HTTPException(status_code=400, detail="Нельзя редактировать done-заявку")
...
async def delete_item(
    ...
    item = result.scalar_one_or_none()
    if not item: raise HTTPException(status_code=404, detail="Заявка не найдена")
    if item.status == "done": raise HTTPException(status_code=400, detail="Нельзя удалить done-заявку")
